...
PUBLIC_API: dict[str, set[str]] = {
    "services.domain.errors": {
        "DomainError", "is_error",
        "IncompatibleUnits", "HardAttributeUnsatisfied", "SubstitutionNotPermitted",
        "OverbuyLimitExceeded", "MixedModeComparison", "BudgetExceeded",
        "PreparationExpired", "DiffNotAccepted", "DiffHashMismatch",
        "QuoteExpired", "QuoteHashMismatch", "QuoteNotConstructible",
        "ApprovalAlreadyConsumed", "ApprovalExpired", "PurchaseAlreadyClaimed",
        "AttemptBlockedByExposure", "PaymentKeyConflict", "VersionConflict",
        "IdempotencyPayloadMismatch", "IllegalTransition",
        "ContradictoryProviderFact", "FactIgnoredStale", "InvalidRecord",
    },
    "services.domain.ids": {
        "ID_PATTERN", "Id", "Digest", "Mode", "Record", "Timestamped",
        "is_valid_id", "utc",
    },
    "services.domain.units": {
        "Dimension", "Unit", "Quantity", "convert", "add", "scale",
        "dimension_of", "base_units_per",
    },
    "services.domain.money": {
        "Currency", "Confidence", "ChargeKind", "Money", "MoneyDelta", "Amount",
        "Charge", "Totals", "BudgetCheck", "compute_totals", "check_budget",
        "total_of", "unit_rate", "difference", "worst",
    },
    "services.domain.canonical": {
        "CANON_SCHEME", "ALL_TAGS", "CanonicalEncodingError",
        "TAG_QUOTE", "TAG_DIFF", "TAG_LINES", "TAG_IDEMPOTENCY",
        "TAG_APPROVAL_ID", "TAG_PAYMENT_KEY", "TAG_PAYMENT_REQUEST",
        "TAG_ORDER_KEY", "TAG_BODY",
        "canonical_json", "digest", "body_hash",
    },
    "services.domain.keys": {
        "approval_id", "payment_key", "payment_request_hash", "order_key", "quote_hash",
        "idempotency_request_hash", "line_hash", "diff_hash",
    },
    "services.domain.transitions": {
        "PaymentState", "OrderState", "RefundState", "DispatchState",
        "ApprovalState", "ClaimState", "JobState", "InboxState", "CaseState",
        "PaymentEvent", "OrderEvent", "RefundEvent", "DispatchEvent",
        "ApprovalEvent", "ClaimEvent", "JobEvent", "InboxEvent", "CaseEvent",
        "PAYMENT_TABLE", "ORDER_TABLE", "REFUND_TABLE", "DISPATCH_TABLE",
        "APPROVAL_TABLE", "CLAIM_TABLE", "JOB_TABLE", "INBOX_TABLE", "CASE_TABLE",
        "MACHINES", "apply", "is_terminal", "StateMachineMisuse",
    },
    "services.domain.provider": {
        "Observed", "PaymentFacts", "OrderFacts", "RefundFacts", "Current",
        "Applied", "ApplyResult",
        "apply_payment_facts", "apply_order_facts", "apply_refund_facts",
    },
    "services.domain.purchase": {
        "PREPARATION_FRESHNESS_SECONDS", "QUOTE_LIFETIME_SECONDS",
        "ChangeKind", "Change", "Diff", "build_diff",
        "Preparation", "QuoteWindow", "AttemptSnapshot", "PurchaseSnapshot",
        "EXPOSED_PAYMENT_STATES",
        "is_fresh", "is_quote_live", "quote_window", "can_build_quote",
        "check_quote_live", "has_unresolved_exposure", "may_create_attempt",
        # the commerce core: what WP-08 writes and WP-09 reads
        "QuoteLine", "CheckoutQuote", "Approval", "Attempt", "ProviderLookup",
        "Purchase", "build_attempt_and_lookup",
    },
    "services.domain.search": {
        "MerchantProgress", "Search", "SearchStatus", "is_current",
    },
    "services.domain.basket": {
        "Comparison", "BasketCost", "compare_baskets", "cheapest", "cheaper_of",
        "BasketLine", "FeeAssessment", "Basket", "build_basket",
    },
    "services.domain.intent": {
        "MAX_ITEMS", "Flexibility", "DeliveryConstraint", "Item", "Intent",
        "Preferences", "UsualBasket", "revise", "touch", "is_current_revision",
        "to_intent_items",
    },
    "services.domain.catalog": {
        "OVERBUY_LIMIT_BP", "BP_DENOMINATOR", "MAX_SEARCH_BASE_UNITS",
        "ExtractionStatus", "Observation", "SubstitutionKind", "Substitution",
        "PackOption", "PackSelection",
        "satisfies_hard_attributes", "check_substitution", "select_packs", "is_usable",
    },
    "services.domain.conversation": {
        "InputKind", "Speaker", "QuestionKind", "PURCHASE_MUTATING_QUESTION_KINDS",
        "QuestionStatus", "VoiceSessionStatus", "Conversation", "Turn", "Question",
        "VoiceSession", "is_expired", "binding_is_current", "may_submit",
    },
    "services.domain.evidence": {
        "FactKind", "EvidenceKind", "InboxEvent", "Evidence", "MissingFact", "Case",
        "is_duplicate", "is_conflicting",
    },
    "services.domain.jobs": {
        "JobType", "PublicationState", "Job", "OutboxEvent", "DuplicateDelivery",
        "retry", "start", "succeed", "fail", "resolve_delivery",
    },
}
