...
describe("AsyncStateView", () => {
  ...
  it("renders stale data alongside a refresh notice", () => {
    const onRetry = vi.fn();
    renderState(
      { status: "stale", data: "milk", error: new ApiError({ kind: "stale_version", message: "x" }) },
      onRetry,
    );
    expect(screen.getByText("data:milk partial:true")).toBeTruthy();
  ...
  it("renders a retryable error with a retry action", () => {
    const onRetry = vi.fn();
    renderState({ status: "error", error: new ApiError({ kind: "network", message: "x" }) }, onRetry);
  ...
  it("does not offer retry for a non-retryable error", () => {
    renderState({ status: "error", error: new ApiError({ kind: "forbidden", message: "x" }) }, vi.fn());
    expect(screen.queryByText("Try again")).toBeNull();
  });

  it("renders an expired state without a retry action", () => {
    renderState({ status: "expired", error: new ApiError({ kind: "expired", message: "x" }) }, vi.fn());
