...
export function PlaybackCard({ text }: PlaybackCardProps) {
  ...
  const { status, caption, audioUrl, autoplayBlocked } = usePollyPlayback(text, audioRef);
  ...
  return (
    <div className="pp-card pp-playback" role="status" aria-label="Assistant voice reply">
      {audioUrl !== null && <audio ref={audioRef} src={audioUrl} className="pp-visually-hidden" />}
