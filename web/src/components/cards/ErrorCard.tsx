...
const MESSAGE_BY_KIND: Record<ApiErrorKind, string> = {
  validation: "Some of the information wasn't valid. Please check and try again.",
  conflict: "That action couldn't be completed because something changed. Refresh and try again.",
  stale_version: "This changed since you last looked. Refresh to see the current version.",
...
export function ErrorCard({ error, onRetry, showRequestId = false }: ErrorCardProps) {
  return (
    <div className="pp-card pp-error" role="alert">
      <p className="pp-error__message">{MESSAGE_BY_KIND[error.kind]}</p>
      {onRetry !== undefined && (
        <button type="button" className="pp-button pp-button--primary" onClick={onRetry}>
          Try again
        </button>
      )}
