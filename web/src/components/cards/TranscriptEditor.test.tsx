...
import { TranscriptEditor, type TranscriptEditorProps } from "./TranscriptEditor";
...
function ControlledHarness(props: Omit<TranscriptEditorProps, "value" | "onChange"> & { initial?: string }) {
...
describe("TranscriptEditor", () => {
  ...
  it("renders whatever the parent's value is - a voice feature can populate it the same way typing does", () => {
    render(<TranscriptEditor value="2 litres of milk" onChange={vi.fn()} onSubmit={vi.fn()} />);
    expect(screen.getByLabelText<HTMLInputElement>("Message").value).toBe("2 litres of milk");
  });

  it("disables the input and send button while submitting", () => {
    render(<TranscriptEditor value="" onChange={vi.fn()} onSubmit={vi.fn()} disabled />);

    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion -- tsc -p tsconfig.json requires this cast (.disabled isn't on plain HTMLElement); eslint's typed-linting disagrees here, a known tsc/eslint type-info mismatch
