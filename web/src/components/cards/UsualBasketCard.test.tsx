...
describe("UsualBasketCard", () => {
  ...
  it("offers to save the last shopper text, and saving reveals a Load usual button", () => {
    render(<UsualBasketCard lastShopperText="2 litres of milk" onLoad={vi.fn()} />);
    fireEvent.click(screen.getByRole("button", { name: "Save this as my usual" }));
  ...
  it("Load usual calls onLoad with the saved text - the same canonical submit path, not a cached result", () => {
    const onLoad = vi.fn();
    render(<UsualBasketCard lastShopperText="2 litres of milk" onLoad={onLoad} />);
    fireEvent.click(screen.getByRole("button", { name: "Save this as my usual" }));
  ...
  it("has no Save button before anything has been said yet", () => {
    ...
    expect(screen.queryByRole("button", { name: "Save this as my usual" })).toBeNull();
  });
});
