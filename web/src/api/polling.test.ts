...
function hiddenUntilCalls(
  visibleAfter: number,
): Visibility & { resumes: number } {
  let checks = 0;
  const state = {
    resumes: 0,
...
describe("pollUntilTerminal", () => {
  ...
  it("polls at 2s for the first attempts, then 5s", async () => {
    ...
    const responses: Job[] = Array.from({ length: 7 }, () => ({
      status: "pending",
    }));
    ...
    await pollUntilTerminal({
      poll: (): Promise<Job> =>
        Promise.resolve(responses[i++] ?? { status: "succeeded" }),
      isTerminal,
      scheduler,
    });
  ...
  it("propagates a non-retryable failure instead of polling a dead job", async () => {
    const poll = vi
      .fn<(signal?: AbortSignal) => Promise<Job>>()
      .mockRejectedValue(new ApiError({ kind: "not_found", message: "gone" }));
    await expect(
      pollUntilTerminal({ poll, isTerminal, scheduler: fakeScheduler() }),
