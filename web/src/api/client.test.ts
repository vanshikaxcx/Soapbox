...
const okHealth = JSON.stringify({
  data: { status: "ok", time: "2026-09-15T13:25:44.293186Z" },
  request_id: "req-1",
});
...
describe("ApiClient envelope handling", () => {
  it("unwraps data and surfaces the request id", async () => {
    const client = new ApiClient({
      baseUrl: "/api",
      transport: envelope(200, okHealth),
    });
  ...
  it("reports a 200 that is not a ProofPath envelope as malformed", async () => {
    const client = new ApiClient({
      baseUrl: "/api",
      transport: envelope(200, JSON.stringify({ status: "ok" })),
    });
...
describe("ApiClient error mapping", () => {
  ...
  it.each(cases)("maps %i/%s to %s", async (status, code, kind) => {
    const client = new ApiClient({
      baseUrl: "/api",
      transport: errorEnvelope(status, code),
    });
  ...
  it("falls back to status when the server sends no usable code", async () => {
    const client = new ApiClient({
      baseUrl: "/api",
      transport: envelope(410, "<html>gateway</html>"),
    });
  ...
  it("treats an unlabelled 409 as a conflict, not a stale version", async () => {
    const client = new ApiClient({
      baseUrl: "/api",
      transport: envelope(409, JSON.stringify({ request_id: "req-1" })),
    });
  ...
  it("marks only transport, rate-limit and server failures retryable", () => {
    ...
    const notRetryable = new ApiError({ kind: "stale_version", message: "x" })
      .retryable;
...
describe("ApiClient mutations", () => {
  it("sends the idempotency key and merges expected_version into the body", async () => {
    ...
    await client.mutate("POST", "/purchases/p1/approve", {
      idempotencyKey: key,
      body: { basket_id: "b1" },
      expectedVersion: 3,
    });
  ...
  it("reuses one key across a retry of the same intent", () => {
    ...
    expect(idempotencyKeyFor("purchase-1", store, uuid)).toBe(
      idempotencyKeyFor("purchase-1", store, uuid),
    );
  });
  ...
  it("attaches the bearer token resolved at request time", async () => {
    ...
    const client = new ApiClient({
      baseUrl: "/api",
      transport,
      accessToken: () => Promise.resolve("tok-1"),
    });
    await client.health();
